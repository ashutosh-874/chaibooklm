...
- Uploaded PDF/VTT files are kept on local disk (`apps/server/uploads/`), read directly by the worker for extraction — `server` and `worker` run on the same machine in this setup.
...
cp .env.example .env              # set OPENAI_API_KEY and JWT_SECRET at minimum
bun install
bun run prisma:generate           # generates the Prisma client (packages/shared/src/generated/prisma)
bun run prisma:migrate            # creates the database schema
